...
use crate::{
    ...
    llvm_sys::core::{
        ...
        llvm_get_inline_asm_has_side_effects, llvm_get_instruction_opcode,
    ...
    match llvm_get_instruction_opcode(inst) {
        ...
        LLVMOpcode::LLVMSelect => {
            ...
            if llvm_can_value_use_fast_math_flags(inst) {
                let fmf = llvm_get_fast_math_flags(inst);
                if !fmf.is_empty() {
                    op.set_attr_llvm_select_fast_math_flags(ctx, fmf.into());
                }
            }
            Ok(op.get_operation())
        }
...
fn convert_global(
    ...
    if let Some(init) = llvm_get_initializer(global) {
        ...
        // We prefer to have an attribute based initializer than a region based one.
        let init_attr = const_llvm_value_to_attr(ctx, cctx, init)?;
        if let Some(attr) = init_attr {
            op.set_initializer_value(ctx, attr);
        } else {
